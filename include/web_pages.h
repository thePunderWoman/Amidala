...
var SCHEMA = [
  ...
  {key:'domeerrlog',label:'Error/Warning Logging',  type:'bool', note:'log RoboClaw-reported faults to the monitor -- off unless debugging'},
...
function load() {
  fetch('/api/config').then(function(r){return r.json();}).then(function(d){
    ...
    _d = allSstr.slice(0, userCnt).map(function(x){return {id:x.id, n:x.n, s:x.s};});
...
function editRow(i, name, s, cat, fav, hid, id) {
  var ob = (i >= 0) ? (i+1) : (_d.length+1);
  var onSave = (i >= 0) ? 'saveEdit('+i+')' : 'saveNew()';
  var idLabel = (id !== undefined && id > 0) ? '<span style="font-size:.6rem;color:var(--muted)">#'+id+'</span>' : '';
  return '<div class="ss-row">'
    +'<div class="ss-idx">'+ob+'<br>'+idLabel+'</div>'
    +'<div class="ss-fields">'
    +'<input type="text" id="ed-name" value="'+esc(name)+'" placeholder="Name" autocomplete="off">'
    +'<input type="text" id="ed-val" value="'+esc(s)+'" placeholder="Serial string" class="mono" autocomplete="off">'
    +'<input type="text" id="ed-cat" value="'+esc(cat)+'" placeholder="Category (optional)" list="cat-list" autocomplete="off">'
...
function render() {
  ...
  if (_d.length === 0 && !_isNew) {
    ...
    _d.forEach(function(row, i) {
      ...
      if (i === _ed && !_isNew) {
        h += editRow(i, row.n, row.s, cat, fav, hid, row.id);
      } else {
        var idBadge = (row.id > 0) ? '<br><span style="font-size:.6rem;color:var(--muted)">#'+row.id+'</span>' : '';
        h += '<div class="ss-row">'
          +'<div class="ss-idx">'+ob+idBadge+'</div>'
          +'<div class="ss-fields">'
  ...
  if (_isNew) {
    var ev = document.getElementById('ed-val'); if (ev) ev.value = '';
    var en = document.getElementById('ed-name'); if (en) { en.value = ''; en.focus(); }
  } else if (_ed >= 0) {
    var el = document.getElementById('ed-name'); if (el) el.focus();
  }
}
...
function saveEdit(i) {
  ...
  postConfig('sstr_'+i, form.n+'|'+form.s).then(function(){
    _d[i] = {id:_d[i].id, n:form.n, s:form.s};
...
function saveNew() {
  ...
  postConfig('sstr_add', form.n+'|'+form.s).then(function(){
    // Push a placeholder so applyFav/Hidden/Cat compute the right 1-based ob
    _d.push({id:0, n:form.n, s:form.s});
    var ob = _d.length;
    applyFav(ob, form.fav);
    applyHidden(ob, form.hid);
    applyCat(ob, form.cat);
    ...
    _isNew=false;
    // Reload to pick up the server-assigned stable ID
