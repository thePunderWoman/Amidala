...
| WCB_Client | ESP-NOW mesh client for joining the droid's WCB network | https://github.com/greghulette/WCBClient — use tag 1.9.6 or later (the SoftAP+STA coexistence fix, see `wcb_client_controller.h`, landed upstream in this release; referenced directly via `lib_deps` in `platformio.ini`, no local clone/fork needed anymore) |
